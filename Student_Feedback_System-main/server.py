...
def predict():
    ...
    write_to_csv_departments(time,teachingscore[0],teaching,courseContentscore[0],courseContent,
                 ...
                 libraryFacilities,extraCurricularscore[0],extraCurricular,emailid,dept,name)
...
def root():
    if not session.get('logged_in'):
        ...
        total_feedbacks, total_positive_feedbacks, total_negative_feedbacks, total_neutral_feedbacks, li,tcse,tit,tece = get_counts()
        tp,tn,tneu,cp,cn,cneu,ep,en,eneu,lwp,lwn,lwneu,lfp,lfn,lfneu,ecp,ecn,ecneu,tcse,tit,tece= li
        ...
        return render_template('admin.html',tf = total_feedbacks,tpf = total_positive_feedbacks,tnegf = total_negative_feedbacks, tneuf= total_neutral_feedbacks,
                               ...
                               tlwneu = tlwneu, tlfp = tlfp, tlfn = tlfn, tlfneu = tlfneu, tecp = tecp,tecn = tecn, tecneu = tecneu,tcse=tcse,tit=tit,tece=tece
                               )
...
def display():
    if not session.get('logged_in'):
        ...
        df = pd.read_csv('dataset/database.csv',on_bad_lines='skip')
